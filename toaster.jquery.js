...
jQuery.toaster = function(settings)
{
	...
	{
		var toasterArea = '<div class="ui-toaster-area-'+settings.position+'"></div>';
		jQuery(settings.base).append(toasterArea);
	}

	if( settings.onlyone==false || (settings.onlyone==true && $(settings.base+' > div.ui-toaster-area-'+settings.position+' div.ui-toaster').length==0) )
	{
		...
		{
			jQuery(settings.base+' > .ui-toaster-area-'+settings.position).prepend(html);
		}
	    else if( settings.position=='tl' || settings.position=='tr' )
		{
			jQuery(settings.base+' > .ui-toaster-area-'+settings.position).append(html);
		}
